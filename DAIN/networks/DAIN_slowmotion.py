...
import DAIN.PWCNet as pwc
import DAIN.S2D_models as s2d
import DAIN.Resblock as res
import DAIN.MegaDepth as mega
import time
...
class DAIN_slowmotion(torch.nn.Module):
    def __init__(self,
        ...
        self.ctxNet = s2d.__dict__['S2DF_3dense']()
        self.ctx_ch = 3 * 64 + 3

        self.rectifyNet = res.__dict__['MultipleBasicBlock_4'](3 + 3 + 3 +2*1+ 2*2 +16*2+ 2 * self.ctx_ch,128)

        self._initialize_weights()
        
        if self.training:
            self.flownets = pwc.__dict__['pwc_dc_net']("DAIN/PWCNet/pwc_net.pth.tar")
        else:
            self.flownets = pwc.__dict__['pwc_dc_net']()
        self.div_flow = 20.0

        #extract depth information
        if self.training:
            self.depthNet=mega.__dict__['HourGlass']("DAIN/MegaDepth/checkpoints/test_local/best_generalization_net_G.pth")
        else:
            self.depthNet=mega.__dict__['HourGlass']()
