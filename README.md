...
<img src="https://github.com/alexfcoding/PyOpenCV/blob/master/images/dain.gif" width="415"/> <img src="https://github.com/alexfcoding/PyOpenCV/blob/master/images/ascii.gif" width="415"/>
...
## Features
- Detect and extract YOLO objects from source (download zip with images and labels)
- Fast and easy test YOLO and Mask R-CNN on different sources
- Apply effects to detected objects and background
- Colorize grayscale source with Caffe model (old movies for example)
- Restore resolution with neural network upscalers ESRGAN / PSNR, EDSR / LapSRN / FSRCNN) selecting pre-trained models from page
- Improve smoothness of video or gif and create slo-mo with Depth-Aware Video Frame Interpolation (DAIN)
- Clean with denoise and sharpen
- Stylize with ASCII drawer, cartoon effect, pencil drawer, sobel filter

## Main page
- Animated demo for rendering modes 
- Launch separate editor process with selected mode, source and unique port for multiple user connections

## Editor page
- Live Flask streaming preview
- Process frames with selected mode / settings and download video
- Take screenshot
- Rewind video file (or Youtube) with slider
- Toggle original / rendered view
- Change render mode and settings without page reload (AJAX)
- View source info, progress and server stats in real-time (RAM, CPU load, FPS, frame size) 
